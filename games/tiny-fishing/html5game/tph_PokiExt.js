...
{
    console.log('[Kasrah] poki_init called - Kasrah Bridge handles initialization');
    poki_gmcallback_pause = _callbackPause;
    poki_gmcallback_unpause = _callbackUnpause;

    // إشعار Kasrah SDK بانتهاء التحميل
    if (window.KasrahSDK && window.KasrahSDK.isInitialized) {
        KasrahSDK.fireEvent('gameLoadingFinished');
    }
    // PokiSDK محاكى في kasrah-poki-bridge.js
    if (window.PokiSDK) {
...
{
    console.log('[Kasrah] poki_gameplayStart');
    if (window.KasrahSDK && window.KasrahSDK.isInitialized) {
...
{
    console.log('[Kasrah] poki_gameplayStop');
    if (window.KasrahSDK && window.KasrahSDK.isInitialized) {
...
function poki_happyTime(magnitude) {
    console.log('[Kasrah] poki_happyTime:', magnitude);
    if (window.KasrahSDK && window.KasrahSDK.isInitialized) {
...
{
    console.log('[Kasrah] poki_commercialBreak called');

    var _onComplete = function() {
        if (poki_gmcallback_unpause && window['gml_Script_' + poki_gmcallback_unpause])
    ...
    if (window.KasrahSDK && window.KasrahSDK.isInitialized) {
        // إيقاف اللعبة
        if (poki_gmcallback_pause && window['gml_Script_' + poki_gmcallback_pause])
            window['gml_Script_' + poki_gmcallback_pause]();

        KasrahSDK.showInterstitial({
            onComplete: function() {
                console.log('[Kasrah] Interstitial completed');
                _onComplete();
            },
            onError: function(err) {
                console.warn('[Kasrah] Interstitial error:', err);
                _onComplete();
            }
        });
    } else {
...
function poki_is_blocked() {
    return false; // Kasrah SDK يتعامل مع حجب الإعلانات داخلياً
}
...
function poki_commercial_break_raw(fn) {
    if (window.KasrahSDK && window.KasrahSDK.isInitialized) {
        KasrahSDK.showInterstitial({
            onComplete: function() { if (fn) fn(true); },
            onError: function() { if (fn) fn(false); }
        });
    } else {
...
function poki_rewarded_break_raw(fn) {
    console.log('[Kasrah] poki_rewarded_break_raw called');
    if (window.KasrahSDK && window.KasrahSDK.isInitialized) {
        KasrahSDK.showRewarded({
            onReward: function() {
                console.log('[Kasrah] Reward earned!');
                if (fn) fn(true);
            },
            onError: function(err) {
                console.warn('[Kasrah] Rewarded error:', err);
                if (fn) fn(false);
            }
        });
    } else {
...
function poki_loadbar(ctx, width, height, total, current, image) {
    // إشعار Kasrah SDK بتقدم التحميل
    if (window.KasrahSDK && window.KasrahSDK.isInitialized) {
        if (current >= total) {
            KasrahSDK.fireEvent('gameLoadingFinished');
        }
    }
    // PokiSDK محاكى - يدعم gameLoadingProgress
    if (window.PokiSDK) {
